      ...
      <div className="bg-card border border-accent/20 rounded-2xl p-6 shadow-lg shadow-black/20">
        ...
        {safeMode && (
          <div className="mt-3 p-3 bg-yellow-500/5 border border-yellow-500/20 rounded-lg flex items-center gap-2">
            <span>⚠️</span>
            <span className="text-yellow-400 text-sm">SAFE MODE ACTIV — Botul nu plasează ordine noi.</span>
          </div>
        )}
          ...
          {chartData.length < 2 ? (
            ...
            <ResponsiveContainer width="100%" height={180}>
              <AreaChart data={chartData}>
                ...
                />
                <Area type="monotone" dataKey="equity" stroke="hsl(160,84%,39%)" strokeWidth={2} fill="url(#eqGradV2)" dot={false} />
              </AreaChart>
            </ResponsiveContainer>
          )}
        </div>
              ...
              {signals.map((s: any) => (
                <div key={s.id} className="flex items-center justify-between py-1.5 border-b border-border-subtle/50 last:border-0">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-xs px-2 py-0.5 rounded-full font-mono ${s.action === "BUY" ? "bg-accent/10 text-accent" : s.action === "SELL" ? "bg-danger/10 text-danger" : "bg-secondary text-muted-foreground"}`}>
                      {s.action}
                    </span>
                  </div>
                  <div className="flex items-center gap-2">
      ...
      <div className="bg-card border border-border-subtle rounded-2xl p-6 shadow-lg shadow-black/20">
        ...
        {loading ? (
          ...
          <div className="text-center py-8">
            ...
            <p className="text-muted-foreground text-sm">Niciun trade înregistrat încă. Botul rulează.</p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border-subtle">
                  {["Simbol","Acțiune","Preț","Qty","P&L","Tip","Data"].map(h => (
                    <th key={h} className={`py-2 px-3 text-xs text-muted-foreground font-body font-normal ${["Preț","P&L","Tip","Data"].includes(h) ? "text-right" : "text-left"} ${h === "Data" ? "hidden sm:table-cell" : ""}`}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
          ...
          <div className="space-y-0.5 font-mono text-xs max-h-[220px] overflow-y-auto scrollbar-hide">
            {logs.map((log: any) => (
              <div key={log.id} className={`flex gap-3 py-1 border-b border-border-subtle/30 ${log.level === "ERROR" ? "text-destructive" : log.level === "WARNING" ? "text-warning" : "text-muted-foreground"}`}>
                ...
                <span className={`shrink-0 px-1.5 rounded text-[10px] border ${log.level === "ERROR" ? "text-destructive border-destructive/20 bg-destructive/10" : log.level === "WARNING" ? "text-warning border-warning/20 bg-warning/10" : "text-accent border-accent/20 bg-accent/10"}`}>
                  {log.level}
                </span>
