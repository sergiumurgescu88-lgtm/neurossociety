...
export default function BotApollo() {
  ...
  return (
    <div className="space-y-8">
      <div className="bg-card border border-violet-500/20 rounded-2xl p-6 shadow-lg shadow-black/20">
        <div className="flex flex-col sm:flex-row sm:items-center gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-heading font-bold">NeuroTrade <span className="text-violet-400">APOLLO</span></h1>
            </div>
          </div>
          <div className="sm:ml-auto flex items-center gap-2">
            ...
            <span className="px-3 py-1 rounded-full text-xs font-mono bg-violet-500/10 border border-violet-500/20 text-violet-400">LIVE</span>
          ...
          {chartData.length < 2
            ...
            : <ResponsiveContainer width="100%" height={180}>
                <AreaChart data={chartData}>
                  <defs><linearGradient id="gBotApollo" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#888" stopOpacity={0.15}/><stop offset="100%" stopColor="#888" stopOpacity={0}/></linearGradient></defs>
                  ...
                  <Area type="monotone" dataKey="equity" stroke="#888" strokeWidth={2} fill="url(#gBotApollo)" dot={false}/>
                </AreaChart>
              </ResponsiveContainer>
          }
        </div>
              ...
              : <div className="space-y-2">{signals.map((s:any)=>(
                  <div key={s.id} className="flex items-center justify-between py-1.5 border-b border-border-subtle/50 last:border-0">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`text-xs px-2 py-0.5 rounded-full font-mono ${s.action==="BUY"?"bg-violet-500/10 text-violet-400":s.action==="SELL"?"bg-danger/10 text-danger":"bg-secondary text-muted-foreground"}`}>{s.action}</span>
                    </div>
                    <span className="text-xs font-mono text-violet-400">{s.confidence??""}%</span>
                  </div>
                ))}</div>
          }
        </div>
      ...
      <div className="bg-card border border-border-subtle rounded-2xl p-6">
        ...
        {loading
          ...
          : trades.length===0
            ? <div className="text-center py-8"><p className="text-2xl mb-2">🌙</p><p className="text-muted-foreground text-sm">BotApollo rulează. Piața e închisă sau confidence-ul nu a fost atins.</p></div>
            : <div className="overflow-x-auto"><table className="w-full text-sm">
                ...
                <tbody>{trades.map(t=>(
                  <tr key={t.id} className="border-b border-border-subtle/50 hover:bg-card-hover/30 transition-colors">
                    ...
                    <td className="py-2.5 px-3"><span className={`px-2 py-0.5 rounded text-xs font-mono ${t.action==="BUY"?"bg-violet-500/10 text-violet-400":"bg-danger/10 text-danger"}`}>{t.action}</span></td>
          ...
          <div className="space-y-0.5 font-mono text-xs max-h-[220px] overflow-y-auto scrollbar-hide">
            {logs.map((log:any)=>(
              <div key={log.id} className={`flex gap-3 py-1 border-b border-border-subtle/30 ${log.level==="ERROR"?"text-destructive":log.level==="WARNING"?"text-warning":"text-muted-foreground"}`}>
                ...
                <span className={`shrink-0 px-1.5 rounded text-[10px] border ${log.level==="ERROR"?"text-destructive border-destructive/20 bg-destructive/10":log.level==="WARNING"?"text-warning border-warning/20 bg-warning/10":"text-violet-400 border-violet-400/20 bg-violet-400/10"}`}>{log.level}</span>
